...
DELIMITER $$
create trigger tr_pedido_descuento
	before insert on `order details`
    for each row 
    begin
		if new.Quantity >10  then set new.Discount=2;
		end if ;
    end 
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE sp_pedidos(in codigo_pedido int)
BEGIN
	IF EXISTS (select* from orders where OrderID=codigo_pedido)
    THEN
		SELECT CustomerID, ShippedDate, ShipCity
		FROM orders 
		where OrderID=codigo_pedido;
	ELSE
		SELECT 'El numero de pedido no existe' as 'Alerta';
	END IF ;
END $$
DELIMITER ;

CALL sp_pedidos(10250);
CALL sp_pedidos(11080);
...
CREATE PROCEDURE sp_listaProductos(in codigo_pedido int)
BEGIN
	SELECT p.ProductName, od.Quantity
    FROM products p 
    inner join `order details` od
    on p.ProductID=od.ProductID
    where codigo_pedido= od.OrderID;
END $$
DELIMITER ;

call sp_listaProductos (10248);

/*16.Crear un procedimiento almacenado que a partir de código de pedido muestre el total a pagar más iva.*/
CREATE PROCEDURE totalConIva(in codigo_pedido int)
BEGIN
	SELECT SUM(((od.UnitPrice*od.Quantity)-od.Discount)*1.12) as 'Total'
    FROM products p
    inner join  `order details` od
    on p.ProductID=od.ProductID
	where codigo_pedido= od.OrderID;
